...
На этом этапе сервер уже работает по адресу: http://127.0.0.1. Можно открыть эту страницу в браузере и подключить второй экран или проектор. Иногда бывает, что 80-й порт для сервера уже занят другой программой - нужно изменить номер порта на другой, указав его при старте: `"C:\Program Files\nodejs\node.exe" server.js 8000`.
